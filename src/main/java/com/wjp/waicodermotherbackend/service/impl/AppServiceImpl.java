...
public class AppServiceImpl extends ServiceImpl<AppMapper, App>  implements AppService{
     ...
     */
    @Override
    public Map<String, String> getCodeSnippets(String sourceDirPath) {
        Map<String, String> codeSnippets = new HashMap<>();

        try {
            File sourceDir = new File(sourceDirPath);
            if (!sourceDir.exists() || !sourceDir.isDirectory()) {
                throw new BusinessException(ErrorCode.SYSTEM_ERROR, "代码目录不存在");
            }

            // 读取HTML文件
            File htmlFile = new File(sourceDir, "index.html");
            if (htmlFile.exists()) {
                codeSnippets.put("html", FileUtil.readUtf8String(htmlFile));
            }

            // 读取CSS文件
            File cssFile = new File(sourceDir, "style.css");
            if (cssFile.exists()) {
                codeSnippets.put("css", FileUtil.readUtf8String(cssFile));
            }

            // 读取JavaScript文件
            File jsFile = new File(sourceDir, "script.js");
            if (jsFile.exists()) {
                codeSnippets.put("javascript", FileUtil.readUtf8String(jsFile));
            }
